...
```{r, results = 'show', purl = FALSE}
widths > 50    # will return logicals with TRUE for the indices that meet the condition
...
```{r, results = 'show', purl = FALSE}
widths[widths < 30 | widths > 50]
widths[widths >= 30 & widths == 21]
```
...
```{r, results = 'show', purl = FALSE}
species <- c("mouse", "rat", "dog", "cat")
species[species == "cat" | species == "rat"] # returns both rat and cat
species %in% c("rat", "cat", "dog", "duck", "goat")
species[species %in% c("rat", "cat", "dog", "duck", "goat")]
```
